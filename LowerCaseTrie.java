...
/**
 * An implementation of a trie which supports .add(s) .contains(s) .remove(s)
 * (remove does not remove nodes, but just removes the information that a word
 * exists at a particular node where the word would be found in the trie)
 * 
 * It also keeps track of the number of distinct words it contains and the
 * number of nodes it is using to hold all those words
 *
